...
class OviTextEncoder(torch.nn.Module):
    def __init__(self, model_name: str = "Wan2.2-TI2V-5B") -> None:
        ...
        self.text_encoder.load_state_dict(
            torch.load(f"./weights/{self.model_name}/models_t5_umt5-xxl-enc-bf16.pth",
                       map_location='cpu', weights_only=False)
        )

        self.tokenizer = HuggingfaceTokenizer(
            name=f"./weights/{self.model_name}/google/umt5-xxl", 
            seq_len=512, 
            clean='whitespace'
        )
...
class OviVAEWrapper(torch.nn.Module):
    def __init__(
        ...
        z_dim_video: int = 48,
        c_dim_video: int = 160,
        video_vae_pth: str = "./weights/Wan2.2-TI2V-5B/Wan2.2_VAE.pth",
        dim_mult: List[int] = [1, 2, 4, 4],
        temperal_downsample: List[bool] = [False, True, True],
        
        audio_mode: str = '16k',
        audio_tod_vae_ckpt: str = "./weights/MMAudio/ext_weights/v1-16.pth",
        audio_bigvgan_ckpt: str = "./weights/MMAudio/ext_weights/best_netG.pt",
    ):
        super().__init__()
